...
## Human Verification Card 示例（未执行）

如果独立工程检查完成后，只剩真实页面行为需要用户确认，Skill 应展示：

```text
请验证：
1. 在测试环境打开订单列表第 3 页。
2. 切换筛选条件，再清空筛选。
3. 预期：两次都回到第 1 页；普通翻页仍保留当前筛选。

证据要求：本项只需回复观察结果；失败可附脱敏截图。
...
这张卡本身不是 Evidence。A 且证据充分后，Skill 才把用户观察写入 `Human Verification H-01` 和一个共享 Evidence batch；B 映射为 FAIL 并返回修复；C 保持 NOT_VERIFIED/BLOCKED。用户不编辑上面的 JSON。
